...
GPT‑6 Astra는 HARU2026의 일상적인 구현 담당자가 아니라 복잡한 의사결정을 돕는 독립 검토자다.
...
- 동시성·경합·성능 병목 검토
- 데이터 저장구조 변경 검토
- 운영 배포 전 위험 및 승인 조건 검토
- 기장과 CC의 판단이 다를 때 독립적인 제3의 판단 제공
- 확인된 사실, 추정, 미검증 항목의 구분

### 기본 모델과 추론 등급

Astra를 사용하는 경우 기본값은 다음과 같다.

- 모델: `gpt-6-astra`
- 기본 추론 등급: `medium`

비용과 토큰 사용량을 고려하여 처음부터 높은 추론 등급을 사용하지 않는다.

`medium` 결과에 중요한 불확실성이 남거나 다음과 같은 고위험 작업일 때만 `high` 또는 `xhigh` 사용을 검토한다.
...
- 심각한 동시성·데이터 손상 가능성
- 대규모 아키텍처 변경
- 운영 장애 원인이 여러 계층에 걸쳐 불명확한 경우

`max` 또는 `ultra`는 기본적으로 사용하지 않는다. 여러 독립 영역을 동시에 검토해야 하는 대규모 장애나 매우 중대한 보안·아키텍처 판단에서만 예외적으로 검토하고, 사용 전에 허대표님께 알린다.

### Astra를 사용하지 않는 작업

다음 작업에는 원칙적으로 Astra를 사용하지 않는다.

- 단순 문구 수정
- 색상·여백·아이콘 등 작은 UI 수정
- 원인이 명확한 단일 버그
- 반복적인 빌드·테스트 실행
- Git status·diff·commit 확인
- 이미 충분히 검토된 동일 대상의 반복 재검토
- 파일명 변경이나 단순 기계적 수정
- 정해진 형식의 문서 정리

이러한 작업은 기장에게 지정된 일반 모델 또는 더 경제적인 모델을 사용한다.

### 사용 결정권

허대표님이 매번 Astra 사용을 직접 지정할 필요는 없다.

시박사 또는 현재 작업을 총괄하는 주 에이전트는 작업의 복잡성, 위험도, 예상 토큰 사용량과 기대 효과를 비교하여 Astra 사용 여부를 결정할 수 있다.

허대표님이 특정 작업 전체를 Astra로 수행하도록 명시한 경우에는 그 지시를 우선한다.

Astra를 사용할 수 없거나 지정한 추론 등급을 사용할 수 없으면 다른 모델로 조용히 대체하지 말고 제한 사항을 보고한다.

### 기본 실행 방식

Astra는 원칙적으로 읽기·분석·판정만 수행한다.

- 기본 권한: read-only 성격의 검토
- 코드나 파일을 직접 수정하지 않음
- main 병합이나 운영 배포를 수행하지 않음
- 외부 서비스 설정을 변경하지 않음
- 결제·인증·데이터를 실제로 변경하지 않음

Astra의 검토 결과를 바탕으로 기장이 실제 코드 수정과 테스트를 수행한다.

Astra가 직접 수정해야 할 특별한 사유가 있으면 작업 범위와 파일을 별도로 지정하고, 다른 에이전트와 같은 파일을 동시에 수정하지 않는다.

### 역할 분담

- 허대표님: 최종 결정과 병합·배포 승인
- 시박사: 기획, 작업 분리, Astra 사용 결정, 결과 종합
- 기장(Codex): 실제 코드 수정, 테스트, Git, PR
- CC: 코드 diff 검토, 회귀·위험 분석
- Astra: 복잡한 구조·보안·성능·운영 판단의 독립 검토

Astra는 기장이나 CC를 대체하지 않는다. 동일 작업을 처음부터 반복 분석하지 않고 기존 결과와 확인된 사실을 전달받아 미해결 쟁점에 집중한다.

### 병렬 작업 안전

- 여러 에이전트가 같은 worktree를 사용해 같은 파일을 동시에 수정하지 않는다.
- Astra는 기본적으로 파일을 수정하지 않는다.
- 병렬 실행은 서로 독립적인 읽기·분석 작업에만 사용한다.
- 코드 수정이 필요한 경우 담당 에이전트와 대상 파일을 명확히 분리한다.
- 기존 미커밋 변경을 삭제·restore·stash하지 않는다.
- 다른 에이전트의 변경을 자기 작업으로 포함하지 않는다.

### Astra 사용 보고

Astra를 사용한 경우 사용자에게 다음을 간단히 보고한다.
...
> Astra Medium 검토 사용\
> 사용 이유: 인증·성능·동시성 문제가 연결된 복합 판단\
> 검토 범위: 변경 코드와 운영 로그의 병목 원인\
> 판정: 확인된 사실 / 추정 / 추가 검증 항목

### 비용·토큰 절약 원칙

- Astra 사용 자체를 작업 품질의 증거로 간주하지 않는다.
- 가장 낮은 추론 등급으로 충분한 결과를 얻는 것을 우선한다.
- 동일 PR이나 동일 HEAD를 이유 없이 반복 검토하지 않는다.
- 전체 저장소 대신 변경 diff와 직접 연결된 코드만 전달한다.
- 팩트시트와 기존 검토 결과를 재사용한다.
- Astra가 필요한 핵심 판단이 끝나면 구현과 반복 검증은 기장에게 돌려준다.
- `medium`으로 충분하면 상위 추론 등급을 사용하지 않는다.

### 프로젝트 모델과의 구분

Astra 협업 정책은 HARU2026 개발 과정의 검토 모델에 관한 것이다.

이 정책은 HARU2026 서비스 내부의 Gemini 모델, Firebase Functions AI 호출 모델 또는 사용자에게 제공되는 AI 모델을 자동으로 변경하지 않는다.

서비스 내부 모델을 변경하려면 별도 작업과 허대표님의 승인이 필요하다.
