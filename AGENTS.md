...
## Work / Codex 작업 배분 원칙 (2026-09-14 추가)

HARU2026 작업 요청을 받으면 시박사가 먼저 작업 성격을 판단하여 Work와 Codex 중 적합한 담당을 지정한다.

- Work: 여러 문서·파일·과거 대화·Git 이력·운영 상태를 넘나드는 조사, 전수감사, 비교분석, 자료정리, 보고서 작성처럼 장시간 다단계 작업에 사용한다.
- Codex(기장): 실제 코드 수정, 테스트, 빌드, Git 작업, PR 생성, 배포 및 기술적 실행 작업에 사용한다.
- 시박사: 작업 범위 설정, Work/Codex 배분, 결과 검토, 위험도 판단, 다음 단계 결정 및 최종 승인 판단을 담당한다.
- 단, 이 최종 승인 판단은 Work/Codex 작업 배분 및 결과 검토 범위에 한하며, main 병합·운영 배포·결제·Firestore 핵심 경로 변경 등 기존 규칙상 허대표님 승인 필수 항목을 대체하지 않는다.
- 조사와 수정이 모두 필요한 경우에는 원칙적으로 `Work 조사 → 시박사 판정 → Codex 수정` 순서를 우선한다.
- 범위가 명확한 버그나 수정 사항은 불필요하게 Work를 거치지 않고 Codex에 바로 배정할 수 있다.
- 허대표님이 별도로 담당을 지정하지 않아도 시박사가 먼저 적합한 작업자를 추천한다.

## Slack 작업 장부 인계 규칙

HARU2026 작업에서 중요한 상태 변화가 생기면 기장의 최종 보고에 반드시 `Slack 기록용 확정 요약`을 포함한다.
...
- 감사 판정·운영 E2E·완료 상태 → #haru2026-감사
- PR·커밋·병합·배포·장애·복구 → #haru2026-개발

주의:

- 기장이 Slack의 공식 감사 판정을 임의로 확정하지 않는다.
- 중간 추측을 확정 사실처럼 작성하지 않는다.
- 비밀번호·토큰·인증정보·결제정보·개인정보를 포함하지 않는다.
- Slack 전송은 시박사가 검토 후 수행한다.
- 동일 내용을 중복 기록하지 않는다.
