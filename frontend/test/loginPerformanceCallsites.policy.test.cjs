...
assert(authContextSrc.includes('bindLoginTraceToAccount(firebaseUser.uid);'));
assert(authContextSrc.includes('endLoginTraceForAuthChange();'));
assertBefore(authContextSrc, "markLoginTrace('T5_user_doc_ready');", 'markLoginAccessReady();');
assertBefore(authContextSrc, "if (pendingDeletion) {", 'markLoginAccessReady();');
...
assert(loginPerformanceSrc.includes("user_doc_retry: ['user_doc_error']"));
assert(loginPerformanceSrc.includes("required_access_ready: ['required_consent_missing', 'pending_deletion']"));
