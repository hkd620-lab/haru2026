...
| HARUraw AI | gemini-2.5-pro (법령 해석/판례) + gemini-3.1-flash-lite-preview (키워드 추출) |
...
### AI분석 버튼 (lawEasyExplain 함수)
- 모델: gemini-2.5-pro
- 출력: 60자 이내 한 문장
- 마크다운 절대 금지 (후처리 강제 제거 필수)
- 60자 초과 시 slice(0,57)+'...' 강제 적용

### 판례 버튼 (lawPrecedent 함수)
- 모델: gemini-2.5-pro
...
### Firestore 저장 필드
- haruraw_query: 사용자 질문
- haruraw_summary: AI 요약
- haruraw_articles: 법조문 텍스트
- haruraw_simple: 질문 + 요약 (제목 추출용)

### Secret Manager 키
- LAW_API_KEY: 법제처 오픈API 인증키
- GEMINI_API_KEY: 기존 키 공유 사용

### 구독 정책 (예정)
- 무료 회원: HARUraw 사용 불가
- 프리미엄(₩4,000/월): 일 5회 제한
- 개발자 UID: 무제한
...
- 다음할일: 수동 제목 입력 기능 사용자 테스트 및 피드백 수집

### 2026-04-13 (12차)
- 완료: SayuModal 제목 편집, RecordDetailModal 복사 기능, AiLibraryPage 카드별 복사 버튼 추가
- 수정파일: frontend/src/app/components/SayuModal.tsx, frontend/src/app/components/RecordDetailModal.tsx, frontend/src/app/pages/AiLibraryPage.tsx
...
- 다음할일: 구독 플랜 분리 (₩3,000 기본 / ₩4,000 HARUraw 포함), 일일 횟수 제한

### 2026-04-13 (14차)
## AI 법률 검색 응답 원칙 (HARUraw)
### 핵심: 질문자의 입장(역할)을 먼저 파악하라

사용자의 질문에서 다음 3가지 역할을 구분하여 해당 입장에 맞는 답변을 제공한다.

| 역할 | 판단 키워드 예시 |
|------|----------------|
| 피해자 | "당했어요", "피해를 입었어요", "신고하고 싶어요" |
| 피고발인(가해자로 지목됨) | "고발당했어요", "억울해요", "나를 신고했어요", "지목됐어요" |
| 제3자(관리자/HR) | "직원이 신고했어요", "어떻게 처리해야 하나요" |

### 답변 구성 방식

1. 질문자의 역할을 먼저 명시한다
   예) "고발을 당하신 입장에서 답변 드립니다."

2. 해당 역할에 맞는 실질적 행동 지침을 우선 제공한다
   - 피고발인이면: 대응 절차, 증거 보존, 법적 권리 등

3. 법조문은 보조 참고자료로만 활용한다
   - 조문 나열이 아닌, 질문자에게 적용되는 내용만 발췌

4. 역할이 불명확하면 먼저 확인 질문을 한다
   예) "피해자 입장이신가요, 아니면 고발을 당하신 입장이신가요?"
---
