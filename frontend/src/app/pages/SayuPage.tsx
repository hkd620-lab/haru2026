...
export function SayuPage() {
  ...
  // HARUraw modal
  const [harurawModal, setHarurawModal] = useState<{ isOpen: boolean; query: string; summary: string; articles: string; recordId: string; shareStatus: string; attachments: { storagePath: string; mimeType: string; fileName: string }[]; }>({
  ...
  useEffect(() => {
    if (!harurawModal.isOpen || harurawModal.attachments.length === 0) return;
    let cancelled = false;
    ...
    (async () => {
      ...
      setHarurawAttachmentUrls((prev) => ({ ...prev, ...map }));
    })();
  ...
  const openFormatSayu = (dateStr: string, formatKey: string, formatLabel: string, recordId?: string) => {
    ...
    if (formatKey === 'haruraw') {
      setHarurawModal({
        ...
        attachments: (record as any).haruraw_attachments || [],
      });
              ...
              return (
                <>
                  <div style={{ ...sectionStyle, backgroundColor: '#F0F4FF', borderColor: '#C7D9F8' }}>
                    ...
                    <p style={{ fontSize: 15, color: '#1A3C6E', fontWeight: 700, lineHeight: 1.6, margin: harurawModal.attachments.length > 0 ? 12 : 0 }}>{harurawModal.query}</p>
                    {harurawModal.attachments.length > 0 && (
                      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
                        {harurawModal.attachments.map((att) => {
                          ...
                          const icon = att.mimeType === 'application/pdf' ? '📄' : att.mimeType === 'text/plain' ? '📝' : '📎';
                          return (
                            <button
                              key={att.storagePath}
                              type="button"
                              ...
                              style={{
                                ...
                                backgroundColor: '#fff', border: '1px solid #C7D9F8', borderRadius: 8,
                                padding: isImage && url ? 4 : '6px 10px', cursor: url ? 'pointer' : 'default',
