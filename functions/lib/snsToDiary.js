...
exports.convertSnsToDiary = (0, https_1.onCall)({
    ...
    const { text, source, timestamp } = (request.data || {});
    if (!text || typeof text !== 'string') {
        throw new https_1.HttpsError('invalid-argument', '게시물 텍스트가 필요합니다.');
    }
    if (text.length > 5000) {
        throw new https_1.HttpsError('invalid-argument', '텍스트는 5000자 이내여야 합니다.');
    }
    let monthlyQuotaReservation = null;
    monthlyQuotaReservation = await (0, monthlyAiQuota_1.reserveMonthlyAiQuota)(request.auth.uid, 'convertSnsToDiary');
