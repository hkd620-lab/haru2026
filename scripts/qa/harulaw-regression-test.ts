...
// 실행: npx tsx scripts/qa/harulaw-regression-test.ts
//
// 배포된 lawSearch 함수를 실제로 호출해 aiSummary 응답을 파일로 저장한다.
// 판정/평가는 하지 않는다. 원문 그대로 수집만 한다.
...
const QUESTIONS: { no: number; title: string; query: string }[] = [
  ...
  { no: 6, title: '교통사고 초기 대응', query: '주차장에서 접촉사고가 났는데 상대방이 블랙박스 영상 공유를 거부하고 과실비율을 다투고 있습니다.' },
...
function getFirebaseApiKey(): string {
  const fromEnv = process.env.VITE_FIREBASE_API_KEY || process.env.FIREBASE_API_KEY;
  if (fromEnv) return fromEnv.trim();

  const candidates = [
    process.env.HARULAW_QA_ENV_PATH,
  ...
  if (!envPath) {
    throw new Error('VITE_FIREBASE_API_KEY를 찾을 수 없습니다. HARULAW_QA_ENV_PATH 또는 환경변수를 지정하세요.');
  }
  return readEnvValue(envPath, 'VITE_FIREBASE_API_KEY');
}
