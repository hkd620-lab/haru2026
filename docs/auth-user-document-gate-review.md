...
- 웹 `AuthContext.tsx`의 진입 판정과 회귀 테스트만 변경한다. 로그인 성능 개선 작업을 재개하지 않는다.
...
승인 후 별도 환경에서 정상 기존 계정, 신규 가입, 동의 누락, 탈퇴유예, 네트워크 실패·재시도, A→B 전환, 로그아웃을 실제 SDK로 검증해야 한다. 약관/개인정보 보기 링크가 최상위 게이트 안의 경로를 다시 여는 기존 구조도 별도 점검 대상으로 남긴다.
